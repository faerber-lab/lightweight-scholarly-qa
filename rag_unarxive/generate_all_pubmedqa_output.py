...
for i,run in enumerate(reversed(runs)):
    ...
    generate_process = subprocess.Popen(["bash", "-c", f"source ~/.bashrc; set_capella_env && python generate_pubmedqa_output.py --topk 10 --output_dir {generate_output_dir}{' --use_rag' if run.use_rag else ''}"])
