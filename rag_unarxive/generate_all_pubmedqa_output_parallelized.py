...
USE_ORIG_DS = False
TEST_SET_ONLY = False
OUTPUT_BASE_DIR = "output_pubmedqa_os_results2"
...
#        for rag_en in RAG_ENABLED:
#            runs.append(RunSetup(f"{base_name}_{lora_name}", BASE_MODELS[base_name], os.path.join(LORA_MODEL_BASE_DIR, lora_dir), rag_en))
...
for i,run in enumerate(reversed(runs)):
    ...
    if USE_ORIG_DS:
        env["USE_ORIG_DS"] = "True"
    if TEST_SET_ONLY:
        env["TEST_SET_ONLY"] = "True"
    ...
    sbatch_process = subprocess.Popen(["bash", "-c", f"source ~/.bashrc; set_capella_env; sbatch -o {log_file} -e {log_file} --job-name=gen_pubmed_mult_{name} generate_pubmedqa_output_parallelized_runner.sbatch"], env=env)
