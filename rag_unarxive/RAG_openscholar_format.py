...
from kg_request import generate_response_kg_request
...
def generate_response(prompt: str, task: Task, context: Any=None, initial: bool=False, previous_chat=None, references: References|None=None, no_rag: bool=False) -> Any:
    ...
    if initial:
        ...
        if task == Task.MULTIQA:
            ...
            context = "You will be shown the text of the abstract, introduction, " \
                      "and conclusion of a scientific paper. Please summarize the " \
                      "key findings of the work in exactly 1 sentence.\n\n Paper text: "
            full_prompt = context + prompt + "\n\n1 Sentence Summary:\n"
            messages = [
                {"role": "user", "content": full_prompt}
            ]
            ...
            if enable_fetch_version:
                ...
                # import here to not slow down measurement tasks
                import nltk
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are a helpful assistant. Please keep your answers short. Exactly follow the users instruction. "
                },
                {"role": "user", "content": full_prompt}
            ]

        elif task == Task.FACT_REQUEST:
            # only give out KG fetch return, could be nicer if put into llama
            return(generate_response_kg_request(prompt))
